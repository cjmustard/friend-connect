...
import (
	...
	"github.com/cjmustard/friend-connect/friendconnect/log"
...
func NewHandler(log *log.ConditionalLogger, accounts *xbox.Store, provider Provider) *Handler {
	if provider == nil {
...
func (m *Handler) syncAndProcess(ctx context.Context) {
	if err := m.Sync(ctx); err != nil {
		m.log.LogFriend("friend sync failed: %v (auto_add=%v auto_accept=%v)", err, m.opts.AutoAdd, m.opts.AutoAccept)
		return
	}
...
func (m *Handler) Sync(ctx context.Context) error {
	...
	m.accounts.WithAccounts(func(acct *xbox.Account) {
		...
		go func(ac *xbox.Account) {
			...
			if err != nil {
				m.log.LogFriend("friend sync failed for %s: %v", tag, err)
			...
			m.log.LogFriend("account sync complete for %s: %d friends", tag, len(friends))
		}(acct)
	})
...
func (m *Handler) autoFollowBack(ctx context.Context) {
	m.accounts.WithAccounts(func(acct *xbox.Account) {
		...
		for _, fr := range friends {
			...
			if fr.Following && !fr.Followed {
				if err := m.provider.AddFriendByXUID(ctx, acct, fr.XUID, fr.Gamertag); err != nil {
					m.log.LogFriend("auto follow-back failed for %s -> %s (%s): %v", tag, fr.Gamertag, fr.XUID, err)
				} else {
					followBackCount++
				}
			}
		...
		if followBackCount > 0 {
			m.log.LogFriend("auto follow-back: %s (%d)", tag, followBackCount)
		}
	})
}
...
func (m *Handler) acceptPending(ctx context.Context) {
	m.accounts.WithAccounts(func(acct *xbox.Account) {
		...
		if err != nil {
			m.log.LogFriend("fetch friend requests failed for %s: %v", tag, err)
			return
		}
		if len(requests) == 0 {
			return
		...
		if err != nil {
			m.log.LogFriend("accept friend requests failed for %s (%d requests): %v", tag, len(requests), err)
			return
		}
		if len(accepted) == 0 {
			accepted = requests
		...
		if len(accepted) > 0 {
			m.log.LogFriend("friend requests accepted: %s (%d)", tag, len(accepted))
		}
	})
}
...
func (m *Handler) logSyncSummary() {
	...
	if totalFriends > 0 {
		m.log.LogFriend("friend sync: %d account(s), %d total friend(s), %d online", accountCount, totalFriends, onlineFriends)
	}
}
