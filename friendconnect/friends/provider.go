...
func (p *XboxProvider) ListFriends(ctx context.Context, acct *xbox.Account) ([]Friend, error) {
	...
	followers, err := p.fetchPeople(ctx, xbox.PeopleFollowersURL, token)
	if err != nil {
		return nil, err
	}
	social, err := p.fetchPeople(ctx, xbox.PeopleSocialURL, token)
	if err != nil {
		return nil, err
	}
...
func (p *XboxProvider) lookupXUID(ctx context.Context, token *xbox.Token, gamertag string) (string, error) {
	endpoint := xbox.ProfileLookupURL(gamertag)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	if err != nil {
		return "", err
	}
...
func (p *XboxProvider) PendingRequests(ctx context.Context, acct *xbox.Account) ([]Request, error) {
	...
	endpoint := xbox.PeopleFriendRequestsURL
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	if err != nil {
		return nil, err
	}
...
func (p *XboxProvider) AcceptRequests(ctx context.Context, acct *xbox.Account, xuids []string) ([]Request, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, xbox.SocialBulkFriendsURL, bytes.NewReader(raw))
	if err != nil {
		return nil, err
	}
	applyCommonHeaders(req, token)
