...
type LoggingOptions = logpkg.LoggingOptions
type ViewershipOptions = session.ViewershipOptions
...
type Options struct {
	...
	Logger *log.Logger
	// Logging controls what types of messages are logged
	Logging logpkg.LoggingOptions
	// ResetTimer defines how often the service should restart to clear ports and client refresh state
...
func (o *Options) ApplyDefaults() {
	...
	if !o.Logging.LogConnections && !o.Logging.LogTransfers && !o.Logging.LogFriends &&
		!o.Logging.LogSessions && !o.Logging.LogNetherNet && !o.Logging.LogErrors {
		o.Logging = logpkg.LoggingOptions{
			LogConnections: true,
			LogTransfers:   true,
			LogFriends:     true,
			LogSessions:    true,
