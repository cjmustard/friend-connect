...
func NewWithOptions(ctx context.Context, opts Options) (*Service, error) {
	...
	// Use logger from LoggingOptions if provided, otherwise use the main Logger field
	loggingOpts := opts.Logging
	if loggingOpts.Logger == nil && opts.Logger != nil {
		loggingOpts.Logger = opts.Logger
	}

	var loggr *log.ConditionalLogger
	if loggingOpts.Logger != nil {
		loggr = log.NewConditionalLogger(os.Stdout, "", 0, loggingOpts)
	} else {
		loggr = log.NewConditionalLogger(os.Stdout, "", 0, loggingOpts)
	}
...
func (s *Service) Run(ctx context.Context) error {
	...
	if addr := s.opts.Relay.RemoteAddress; addr != "" {
		if foreign, err := minecraft.NewForeignStatusProvider(addr); err == nil {
			...
			s.log.LogError("relay status provider unavailable: %s - %v", addr, err)
		}
	}
...
func (s *Service) resetService(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			s.log.LogError("reset timer triggered, restarting service")
			...
			s.log.LogError("service reset complete")
		}
	}
}
