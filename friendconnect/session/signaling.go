...
import (
	...
	logpkg "github.com/cjmustard/friend-connect/friendconnect/log"
...
func (n *notifier) NotifySignal(signal *nethernet.Signal) {
	...
	case nethernet.SignalTypeError:
		if signal.Data != "" {
			n.log.LogNetherNet("nethernet connection error: %s (connection: %d) - %s", n.tag, signal.ConnectionID, signal.Data)
		} else {
			n.log.LogNetherNet("nethernet connection error: %s (connection: %d)", n.tag, signal.ConnectionID)
		}
	}
}
...
func (n *notifier) NotifyError(err error) {
	if err != nil && !errors.Is(err, nethernet.ErrSignalingStopped) && n.log != nil {
		n.log.LogNetherNet("nethernet signaling error: %s - %v", n.tag, err)
	}
	n.once.Do(func() {
		close(n.done)
	})
...
func NewSignalingHub(logger *logpkg.ConditionalLogger, accounts *xbox.Store) *SignalingHub {
	if logger == nil {
		logger = logpkg.NewConditionalLogger(os.Stdout, "", 0, logpkg.LoggingOptions{})
	}
	return &SignalingHub{
		log:      logger,
		accounts: accounts,
...
func (m *SignalingHub) runSession(ctx context.Context, acct *xbox.Account, sess *SignalingSession) {
	...
	for {
		...
		if src == nil {
			m.log.LogError("missing token source for %s", acct.Gamertag())
			return
		}
		d := signaling.Dialer{
		...
		if err != nil {
			...
			m.log.LogNetherNet("dial nethernet failed for %s: %v", acct.Gamertag(), err)
