...
func NewServer(logger *logpkg.ConditionalLogger, accounts *xbox.Store, netherHub *SignalingHub, httpClient *http.Client) *Server {
	if httpClient == nil {
...
func (m *Server) startAccount(acct *xbox.Account) {
	...
	go func() {
		if err := m.ensureSession(ctx, acct); err != nil {
			m.log.LogSession("create session failed for %s: %v", acct.Gamertag(), err)
		}
	}()
