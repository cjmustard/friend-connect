...
func (m *Server) buildStatus(ctx context.Context, acct *xbox.Account, tok *xbox.Token) (room.Status, error) {
	hostName := m.getDefaultValue(m.viewership.HostName, defaultHostName(acct.Gamertag()))
	worldName := m.getDefaultValue(m.viewership.WorldName, defaultWorldName(acct.Gamertag()))
...
func (m *Server) getDefaultValue(value, defaultValue string) string {
	if value == "" {
		return defaultValue
	}
	return value
...
// handleConn processes a new client connection.
func (m *Server) handleConn(ctx context.Context, conn *minecraft.Conn) {
	...
	host := m.waitForHostIfRequired(ctx, addr, conn)
	if host == nil {
		return // Connection was closed or rejected
	}

	m.setupClientSession(addr, host, conn)
	...
	m.waitForConnectionClose(ctx, addr, conn, host)
}
...
// waitForHostIfRequired waits for a transfer flag if relay is enabled.
func (m *Server) waitForHostIfRequired(ctx context.Context, addr string, conn *minecraft.Conn) *xbox.Account {
...
func (m *Server) setupClientSession(addr string, host *xbox.Account, conn *minecraft.Conn) *ClientSession {
	return m.trackClient(addr, host, conn)
}
...
func (m *Server) waitForConnectionClose(ctx context.Context, addr string, conn *minecraft.Conn, host *xbox.Account) {
	select {
	case <-ctx.Done():
		return
...
func (m *Server) transferClient(ctx context.Context, conn *minecraft.Conn) error {
	...
	select {
	case <-ctx.Done():
	case <-time.After(transferWaitTimeout):
	}
	return nil
}
